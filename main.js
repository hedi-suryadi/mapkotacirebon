...
polygon: {
  ...
  var ctSidebar=L.control.sidebar('sidebar_div', {
   ...
   // <!-- Lokasi saya via GPS/Geolokasi -->
   var ctEasybtn=L.easyButton('<img src="target.svg" style="width:14px"><span>&start;</span>',
   function() {
   map.locate({setView: true})
   }).setPosition('topcenter').addTo(map);
   var IconMYLOC = L.icon({
   iconUrl: 'iamhere.svg',
   iconSize: [50, 50]
   });
   ...
   L.marker(e.latlng, {icon:IconMYLOC}).addTo(map)
